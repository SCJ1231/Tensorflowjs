...
// 모델 생성 1  model = createConvMoedel()
function createConvModel(){
  // 합성곱 신경망의 첫번째 층은 28 x 28 픽셀의 흑백이미지를 받음
  // 커널크기가 3픽셀인 16개의 필터사용, 렐루 활성화
  const model = tf.sequential()
  model.add(tf.layers.conv2d({
    inputShape: [IMAGE_H, IMAGE_W, 1], // 체널 1개 흑백
    kernelSize: 3, // 창의 크기
    filters: 16, // 창의 갯수
    activation: 'relu'
  }))
  ...
  model.add(tf.layers.flatten({})) // 1열로 평평하게 편다. 28 x 28 (784)
...
function createDenseModel() {
  const model = tf.sequential()
  model.add(tf.layers.faltten({inputShape: [IMAGE_H, IMAGE_W, 1]}))
  model.add(tf.layers.dense({  units: 42, activation: 'relu'}))
  model.add(tf.layers.dense({  units: 42, activation: 'softmax'}))
  return model
}
